...
// that before even requesting feed.php).
////////////////////////////////
...
if (isset($_GET['key']) && ($key_index = array_search($_GET['key'], $options->api_keys)) !== false) {
	...
	$redirect = 'http://'.htmlspecialchars($host.$path).'/feed.php?url='.urlencode($_qs_url);
...
if ($options->caching) {
	...
	if ($options->apc && $options->smart_cache) {
		apc_add("cache.$cache_id", 0, 30*60);
...
function get_cache() {
	...
	if ($cache === null) {
		$frontendOptions = array(
			'lifetime' => 30*60, // cache lifetime of 10 minutes
